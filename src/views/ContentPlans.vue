  ...
  <section class="plans-page" dir="rtl">
            ...
            <tr v-for="plan in plans" v-else :key="plan.id">
              ...
              <td>
                <div class="milestone" style="min-width: 175px;">
                  <div style="display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <button v-if="isManager && plan.status !== 'completed'" title="تنبيه واتساب الذكي" class="wa-quick-btn" type="button" @click="smartNotify(plan)">
              ...
              <td v-if="isManager">
                <div class="actions-cell">
                  <button class="action edit" type="button" title="تعديل" @click="openManagerModal(plan)">✎</button>
    ...
    <div v-if="showDeliverySuccessModal" class="modal-overlay" @click.self="closeDeliveryModal">
      <div class="modal-content delivery-modal" role="dialog" style="width: min(500px, 100%) !important; text-align: center;">
        ...
        <p style="margin-bottom: 20px;">الآن يمكنك إبلاغ العميل بالرابط، وإعطاء إشارة البدء لفريق التنفيذ.</p>
        ...
        <div class="wa-actions-container" v-if="deliveredPlan">
          <a :href="generateWaLink('client_delivery', deliveredPlan)" target="_blank" class="wa-btn-large" style="text-decoration:none;">
            <span>إبلاغ العميل على واتساب</span> <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M17.472...Z"/></svg>
          </a>
          ...
          <div v-if="getExecutors(deliveredPlan).length > 0" class="executors-list mt-3">
            ...
            <div style="display:flex; flex-direction:column; gap:8px;">
              <a v-for="executor in getExecutors(deliveredPlan)" :key="executor.id" 
                 :href="generateWaLink('executors_start', deliveredPlan, executor)" target="_blank" 
                 class="wa-btn-small" style="text-decoration:none; display:block; text-align:center;">
                إرسال للمنفذ: {{ executor.name }} 📱
              </a>
            </div>
          </div>
    ...
    <div v-if="showHistoryModal" class="modal-overlay" @click.self="closeHistoryModal"><div class="modal-content history-modal" style="width: min(500px, 100%) !important;"><button class="modal-close" type="button" @click="closeHistoryModal">×</button><div class="modal-icon">📋</div><h3>السجل الكامل للحركات</h3><div class="history-timeline"><div v-for="(history, index) in getReviewHistories(selectedHistoryPlan)" :key="index" class="timeline-item"><div class="tl-dot" :class="history.action === 'approved' ? 'tl-green' : 'tl-red'"></div><div class="tl-content"><div class="tl-header"><strong>{{ history.reviewer?.name || 'مجهول' }}</strong><span class="tl-date" dir="ltr">{{ formatDate(history.created_at) }}</span></div><div class="tl-action" :class="history.action === 'approved' ? 'text-green' : 'text-red'">{{ history.action === 'approved' ? '✅ وافق على الخطة' : '❌ رفض الخطة' }}</div><div class="tl-notes" v-if="history.notes">{{ history.notes }}</div></div></div></div></div></div>
    <div v-if="showFollowUpsModal" class="modal-overlay" @click.self="closeFollowUpsModal"><div class="modal-content followups-modal" role="dialog" style="width: min(650px, 100%) !important;"><button class="modal-close" type="button" @click="closeFollowUpsModal">×</button><div class="modal-icon" style="background: linear-gradient(145deg, #8fc9ff, #65b5ff); color: #12183f;">💬</div><span class="eyebrow" style="color: #65b5ff;">سجل المتابعة</span><h3>متابعة العميل: {{ selectedFollowUpPlan?.client?.name }}</h3><form class="plan-form mt-2" style="background: rgba(6,11,37,.3); padding: 15px; border-radius: 12px; border: 1px solid rgba(145,160,230,.1);" @submit.prevent="saveFollowUp"><div class="form-group mb-0"><label>{{ editingFollowUpId ? 'تعديل التحديث' : 'إضافة تحديث جديد' }}</label><textarea v-model="followUpForm.content" rows="3" required></textarea></div><div class="modal-actions" style="margin-top: 10px;"><button v-if="editingFollowUpId" type="button" class="secondary-btn" @click="cancelEditFollowUp">إلغاء التعديل</button><button type="submit" class="primary-btn" style="background: linear-gradient(110deg, #8fc9ff, #65b5ff);" :disabled="actionLoading === 'save-followup'">{{ actionLoading === 'save-followup' ? 'جارٍ...' : (editingFollowUpId ? 'حفظ التعديل' : 'إضافة التحديث') }}</button></div></form><hr class="separator" /><div class="history-timeline"><div v-if="getFollowUps(selectedFollowUpPlan).length === 0" class="muted text-center">لا توجد متابعات مسجلة حتى الآن.</div><div v-for="followUp in getFollowUps(selectedFollowUpPlan)" :key="followUp.id" class="timeline-item"><div class="tl-dot" style="background: #65b5ff;"></div><div class="tl-content fu-content"><div class="tl-header"><strong>{{ followUp.user?.name || 'مجهول' }}</strong><span class="tl-date" dir="ltr">{{ formatDate(followUp.created_at) }}</span></div><div class="fu-text">{{ followUp.content }}</div><div class="fu-actions" v-if="isManager || currentUserId === followUp.user_id"><button type="button" @click="editFollowUp(followUp)" class="fu-btn edit-fu">✎ تعديل</button><button type="button" @click="deleteFollowUp(followUp.id)" class="fu-btn delete-fu">⌫ حذف</button></div></div></div></div></div></div>
...
const getPlanStatusInfo = (status) => {
  switch(status) {
    ...
    case 'reviewed': return { text: 'جاهزة للتسليم', class: 'status-blue' };
...
const triggerWaPrompt = (title, desc, type, plan) => {
  ...
  if (!link) {
    showToast('رقم الهاتف غير مسجل أو غير صالح للشخص المستهدف.');
    return;
  }
...
const cancelEditFollowUp = () => { editingFollowUpId.value = null; followUpForm.content = ''; };
const editFollowUp = (item) => { editingFollowUpId.value = item.id; followUpForm.content = item.content; };
const saveFollowUp = async () => { if (!followUpForm.content.trim()) return; actionLoading.value = 'save-followup'; try { if (editingFollowUpId.value) { await api.put(`/follow-ups/${editingFollowUpId.value}`, { content: followUpForm.content }); showToast('تم التعديل'); } else { await api.post(`/content-plans/${selectedFollowUpPlan.value.id}/follow-ups`, { content: followUpForm.content }); showToast('تمت الإضافة'); } cancelEditFollowUp(); await fetchPlans(); selectedFollowUpPlan.value = plans.value.find(p => p.id === selectedFollowUpPlan.value.id); } catch (error) {} finally { actionLoading.value = ''; } };
